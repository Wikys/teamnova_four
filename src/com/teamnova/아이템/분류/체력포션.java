...
public class 체력포션 extends 포션{
    ...
    @Override
    public void 아이템_효과() { //체력포션 회복
        if (유저명.전투중체력 == 유저명.최대체력){ // 최대 체력,마나가 현재 체력마나와 같으면 회복없음
            System.out.println("최대 체력인데 왜먹는지 모르겠다..");
        }
        else if(this.수치+유저명.전투중체력 > 유저명.최대체력){ // 포션회복량으로 최대체력 넘길거같으면 그냥 최대체력 리턴
            유저명.전투중체력 = 유저명.최대체력;
        }else{
            유저명.전투중체력 = 유저명.전투중체력 + this.수치; // 아니면 그냥 회복
        }
    ...
    @Override
    public void 아이템_장착해제() { //메인메뉴 클래스에서 처리
